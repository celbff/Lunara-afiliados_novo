...
const createSession = async (userId, token, refreshToken, req) => {
  ...
  // Hash dos tokens para segurança
  ...
  await pool.query(`
    ...
    VALUES ($1, $2, $3, $4, $5, $6)
  `, [userId, tokenHash, refreshTokenHash, expiresAt, req.get('User-Agent'), req.ip]);
};
...
router.post('/logout-all', authenticate, async (req, res, next) => {
  try {
    ...
    logger.info(`Logout de todos os dispositivos: ${req.user.email}`);
    res.json({
      success: true,
...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    // Hash do refresh token para busca
    const refreshTokenHash = crypto.createHash('sha256').update(refreshToken).digest('hex');
    ...
    await pool.query(`
      ...
      WHERE id = $4
    `, [newTokenHash, newRefreshTokenHash, expiresAt, session.id]);
    res.json({
      success: true,
      data: {
        token: newToken,
...
router.post('/change-password', authenticate, changePasswordValidation, async (req, res, next) => {
  try {
    ...
    // Invalidar todas as sessões existentes (exceto a atual)
    ...
    logger.info(`Senha alterada pelo usuário: ${req.user.email}`);
    res.json({ 
      success: true, 
      message: 'Senha alterada com sucesso. Você foi desconectado dos outros dispositivos.' 
    });
  } catch (error) {
    next(error);
  }
